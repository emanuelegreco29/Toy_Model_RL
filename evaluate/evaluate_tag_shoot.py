...
def load_latest_policies(base_dir='policies/Tag_Shoot_SP'):
    folders = glob.glob(os.path.join(base_dir, 'Tag_Shoot_SelfPlay_*'))
    if not folders:
        raise FileNotFoundError(f"No Tag_Shoot_SelfPlay folders found in {base_dir}")
    ...
    print(f"Chaser policy: {chaser_file}")
    print(f"Evader policy: {evader_file}")
...
def animate_trajectory(ch_traj, ev_traj, filename, save_dir):
    ...
    def update(frame):
        ...
        min_pt = pts.min(axis=0); max_pt = pts.max(axis=0)
...
save_dir = os.path.join('plots/Tag_Shoot', f'eval_tag_shoot_{ts}')
os.makedirs(save_dir, exist_ok=True)

print("Evaluating TagShoot agents for 100 episodes...")

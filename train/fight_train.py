...
os.makedirs(f"policies/Fight_SP/Fight_SelfPlay_{ts}", exist_ok=True)

# --- Hyperparameters ---
gamma, lam           = 0.99, 0.95
batch_size           = 2048
total_epochs         = 500
save_interval        = 25 # Save every X epochs
...
# --- Environment ---
env = FightEnv(K_history=K_history)
...
for ep in range(1, total_epochs+1):
    ...
    steps = 0
    config = infos[env.agents[0]]['config']
    ...
    # calcolo reward totali per ciascun agente
    total_rews = {
        ag: sum(r.item() for r in buf[ag]['rews'])
        for ag in env.agents
    }
    # leggo HP correnti
    hp_now = { ag: env.hp[ag] for ag in env.agents }
    
    print(
        f"[Episode {ep}] "
        f"Config= {config} | "
        f"Reward {a0}: {total_rews[a0]:.2f}, {a1}: {total_rews[a1]:.2f} | "
        f"HP {a0}: {hp_now[a0]}, {a1}: {hp_now[a1]}"
    )
    
    if ep % save_interval == 0:
        for ag in env.agents:
            torch.save(agents[ag].state_dict(),
                    f"policies/Fight_SP/Fight_SelfPlay_{ts}/{ag}_policy_ep{ep}.pth")
        torch.save(critic.state_dict(),
                f"policies/Fight_SP/Fight_SelfPlay_{ts}/critic_ep{ep}.pth")

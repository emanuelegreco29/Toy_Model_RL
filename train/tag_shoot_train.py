...
os.makedirs(f"policies/Tag_Shoot_SP/Tag_Shoot_SelfPlay_{ts}", exist_ok=True)
...
for ep in range(1, total_epochs+1):
    ...
    chaser_hp = infos[a0]['Chaser HP']
    evader_hp = infos[a1]['Evader HP']
    print(f"[Episode {ep}] Distance {dist:.3f} | Chaser HP: {chaser_hp}, Evader HP: {evader_hp}")

    if ep % save_interval == 0:
        for ag in env.agents:
            torch.save(agents[ag].state_dict(),
                    f"policies/Tag_Shoot_SP/Tag_Shoot_SelfPlay_{ts}/{ag}_policy_ep{ep}.pth")
        torch.save(critic.state_dict(),
                f"policies/Tag_Shoot_SP/Tag_Shoot_SelfPlay_{ts}/critic_ep{ep}.pth")

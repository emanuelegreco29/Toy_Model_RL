...
from ..utilities.utilities import convert, get_grad_norm
...
class PPOTrainer():
    ...
    def ppo_update(self, policy: PPOPolicy, sample):
        ...
        old_action_log_probs_batch = convert(old_action_log_probs_batch).to(**self.tpdv)
        advantages_batch = convert(advantages_batch).to(**self.tpdv)
        returns_batch = convert(returns_batch).to(**self.tpdv)
        value_preds_batch = convert(value_preds_batch).to(**self.tpdv)
        ...
        if self.use_max_grad_norm:
            ...
            actor_grad_norm = get_grad_norm(policy.actor.parameters())
            critic_grad_norm = get_grad_norm(policy.critic.parameters())

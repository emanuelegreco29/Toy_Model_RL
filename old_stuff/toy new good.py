...
class PointMassEnv(gym.Env):
    ...
    def compute_reward(self, prev_state, current_state, target):
        ...
        # 1) Ricompensa proporzionale se la distanza è diminuita
        ...
        # 3) Penalità se molto lontano
        if curr_distance > 50.0:
            reward -= 2.0

        # 4) Penalità per coordinate negative (se non vuoi zone negative)
        for i in range(3):
            if curr_pos[i] < 0:
                reward -= 1.0
...
def train_dqn(obs_dim=5, action_dim=6, learning_rate=0.001, gamma=0.95, buffer_capacity=100000, batch_size=128,  total_steps=100000, target_update_interval=500, train_freq=4):
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=10000, gamma=gamma)

    replay_buffer = ReplayBuffer(capacity=buffer_capacity)

    epsilon_start = 1.0
    epsilon_final = 0.05
    epsilon_decay = 0.9 # frazione di total_timesteps durante cui ε scende da start a final
...
def train_and_plot(obs_dim=5, action_dim=6, learning_rate=0.001, gamma=0.95, 
    ...
    epsilon_start = 1.0
    epsilon_final = 0.05
    epsilon_decay = 0.5  # frazione di total_steps durante cui ε decresce da start a final
    epsilon_by_timestep = lambda t: epsilon_final + (epsilon_start - epsilon_final) * \
        np.exp(-t / (total_steps * epsilon_decay))
    ...
    for t in range(1, total_steps+1):
        ...
        replay_buffer.push(state, action, reward, next_obs, done)
        state = next_obs
        episode_reward += reward

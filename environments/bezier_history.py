...
from reward_functions.predictive_negative import compute_reward as predictive_reward
...
class PointMassEnv(gym.Env):
    ...
    def __init__(self, K_history: int = 1, reward_fn='predictive'):
        super().__init__()
        # History length for past K target states
        self.K_history = K_history
        # Reward function choice
        if reward_fn == 'predictive':
            self.compute_reward = predictive_reward
        # Movement parameters
    ...
    def step(self, action):
        ...
        reward = self.compute_reward(self, prev_agent, self.state)

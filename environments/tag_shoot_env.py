...
class TagShootEnv(ParallelEnv):
    ...
    def __init__(self, K_history: int = 1):
        ...
        self.chaser_v_max = 1.5
        self.chaser_v_min = 1.0
        self.evader_v_max = 1.2
        self.evader_v_min = 0.7
    ...
    def reset(self, seed=None, options=None):
        ...
        for ag in self.agents:
            ...
            infos[ag] = {
                ...
                'Chaser HP': int(self.hp['Agent 0']),
                'Evader HP': int(self.hp['Agent 1']),
    ...
    def _get_obs(self, agent):
        ...
        enemy_hp = self.hp[self._other(agent)]
        ...
        obs = np.concatenate([
            ...
            np.array([enemy_hp], dtype=np.float32) # 1
    ...
    def step(self, action_dict):
        ...
        for ag, act in action_dict.items():
            ...
            # Update state
            if ag == 'Agent 0': # chaser
                v     = np.clip(v + dv, self.chaser_v_min, self.chaser_v_max)
            else: # evader
                v     = np.clip(v + dv, self.evader_v_min, self.evader_v_max)
        ...
        for ag in self.agents:
            ...
            if self._is_behind(ag):
                ...
                self.behind_enemy[ag] += 1
            else:
                self.tracking_counter[ag] = 0
                self.behind_enemy[ag] = 0

            rewards[ag] = self._compute_reward(ag)
            
        # Shooting
        # Tie-breaker pro-chaser, no double-KO
        candidates = []
        for ag in self.agents:
            other = self._other(ag)
            if self.hp[other] > 0 and self.behind_enemy[ag] >= 5:
        ...
        for ag in candidates:
            ...
            self.hp[other] = max(0, self.hp[other] - 10)

            if self.hp[other] == 0:
                break
        
        term = (self.current_step >= self.max_steps or self.hp['Agent 0'] <= 0 or self.hp['Agent 1'] <= 0)

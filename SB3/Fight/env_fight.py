...
class PointMassEnv(gym.Env):
    ...
    calcolato on-the-fly punto per punto, con gestione di HP e danno al target.
    """
    dt = 0.1
    max_steps = 500
    ...
    def __init__(self, K_history: int = 1):
        ...
        # --- Parametri WEZ e armi ---
        self.wez_length = 2.0  # lunghezza massima del cono di ingaggio
        self.wez_angle = math.radians(30)  # angolo (metà apertura) del cono in radianti
        self.wez_cos_threshold = math.cos(self.wez_angle)  # soglia coseno per il cono
        self.hp_decrement = 5  # HP tolti al target ad ogni ingaggio completo
        self.wez_reward = 0.1  # reward aggiuntivo (positivo) ad ogni hit
        self.wez_step = 0.05 # reward aggiuntivo (positivo) per step in WEZ
        self.destroy_bonus = 2.0  # bonus reward (positivo) alla distruzione
        ...
        # L'osservazione ora include anche gli HP del target
        obs_dim = 18
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Inizializza HP e stato di ingaggio
        self.target_hp = 100  # HP iniziali del target
        self.wez_steps = 0    # contatore di step consecutivi in WEZ
        self.target_destroyed = False  # flag di target abbattuto
    ...
    def _get_obs(self):
        ...
        # velocità e direzione normalizzata del target
    ...
    def _in_wez(self):
        # Verifica se il target è all'interno del cono WEZ dell'agente
        agent_pos = self.state[:3]
        target_pos = self.target_state

        # Vettore dal punto agente al target
        ...
        # Direzione agent (vettore unitario basato su yaw e pitch)
        yaw, pitch = self.state[4], self.state[5]
        dx = math.cos(pitch) * math.cos(yaw)
        dy = math.cos(pitch) * math.sin(yaw)
        dz = math.sin(pitch)
        direction_vec = np.array([dx, dy, dz])

        # Proietto verso il target
        rel_unit = vec / dist
        # Se il coseno dell'angolo tra axis e vettore al target è sopra soglia
    ...
    def step(self, action):
        # Avanza target nella traiettoria
        ...
        self.current_step += 1
        reward = self.compute_reward()  # reward base in [-1,0]
        ...
        # Verifico presenza del target nel WEZ e aggiorno contatore
        if self._in_wez():
            self.wez_steps += 1
            reward += self.wez_step  # aggiungo reward per step in WEZ
        else:
            self.wez_steps = 0

        # Se 10 step consecutivi in WEZ, infliggo danno e assegno reward addizionale
        if self.wez_steps >= 5 and self.target_hp > 0:
            self.target_hp = max(self.target_hp - self.hp_decrement, 0)  # rimuovo HP
            reward += self.wez_reward
            self.wez_steps = 0  # reset del contatore

            # Se HP arriva a zero, target è distrutto
            if self.target_hp <= 0:
                self.target_destroyed = True
                reward += self.destroy_bonus
 
        # Determino terminazione
        terminated = self.target_destroyed
        truncated = (not terminated) and (self.current_step >= self.max_steps)

        # Costruisco info da restituire
        info = {
            "distance": float(np.linalg.norm(self.state[:3] - self.target_state)),
            "followed": int(self.totally_behind),
            "target_destroyed": self.target_destroyed
        }
        # Se episodio finito (per distruzione o per timeout), aggiungo HP finale
        if terminated or truncated:
            info["final_hp"] = int(self.target_hp)

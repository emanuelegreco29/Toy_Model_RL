...
class PointMassEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.delta_z = 0.25
        # rimane per uso interno se servisse
        ...
        self.continuous_action_space = spaces.Box(low_act, high_act, dtype=np.float32)
...
class DiscretePointMassEnv(PointMassEnv):
    """
    Wrapper che mappa un'azione discreta [0 .. n_theta*n_z-1] nei due parametri continui (dtheta, dz).
    """
    def __init__(self, n_theta=11, n_z=11):
        super().__init__()
        ...
        # action space piatto
        self.action_space = spaces.Discrete(n_theta * n_z)

    def step(self, action):
        # action è un int in [0 .. n_theta*n_z-1]
        idx = int(action)
        idx_th = idx // self.n_z
        idx_z  = idx % self.n_z
        dtheta = self.theta_bins[idx_th]
        dz     = self.z_bins[idx_z]
        return self._step_continuous(np.array([dtheta, dz], dtype=np.float32))
...
class PointMassEnv(gym.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        # Stima posizione e direzione iniziale del target
        p0 = self.target_traj[0].copy() # Posizione iniziale del target
        p1 = self.target_traj[1].copy()
        dir_vec = p1 - p0 # Vettore direzione del target
        dir_unit = dir_vec / (np.linalg.norm(dir_vec) + 1e-8)

        # Scegli casualmente uno dei 4 scenari
        scenario = np.random.randint(4)

        if scenario == 0:
            # 0) agent dietro il target di 2 unità, fronte comune
            agent_pos = p0 - 2.0 * dir_unit
            yaw  = np.arctan2(dir_unit[1], dir_unit[0])
            pitch = np.arcsin(dir_unit[2])
        elif scenario == 1:
            # 1) agent davanti al target di 2 unità, fronte comune
            agent_pos = p0 + 2.0 * dir_unit
            yaw  = np.arctan2(dir_unit[1], dir_unit[0])
            pitch = np.arcsin(dir_unit[2])
        elif scenario == 2:
            # 2) agent dietro di 2, ma “faccia a faccia” (orientamento opposto)
            agent_pos = p0 - 2.0 * dir_unit
            yaw  = np.arctan2(-dir_unit[1], -dir_unit[0])
            pitch = -np.arcsin(dir_unit[2])
        else:
            # 3) agent davanti di 2, “schiena a schiena” (orientamento opposto)
            agent_pos = p0 + 2.0 * dir_unit
            yaw  = np.arctan2(-dir_unit[1], -dir_unit[0])
            pitch = -np.arcsin(dir_unit[2])

        # Inizializza lo storico del target (K_history+1 copie di p0)
        self.target_history = deque([p0] * (self.K_history + 1), maxlen=self.K_history + 1)
        self.target_state   = p0.copy()

        # Imposta lo stato iniziale dell’agent
        # [x, y, z, velocità, yaw, pitch]
        self.state = np.array([
